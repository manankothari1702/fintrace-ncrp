...
|---|------|--------|----------------|
| 1 | Version bumped across root/backend/frontend `package.json` | **[x] PASS** | Bumped root `package.json`, `backend/package.json`, and `frontend/package.json` from `0.2.0` → **0.3.0**; sidebar UI footer reads `v0.3.0` (`APP_VERSION` in `frontend/src/components/Sidebar.jsx`); `app.getVersion()` (IPC `app:get-version`) and the backend `appVersion()` both read `package.json`, so they return 0.3.0 automatically. Installer-name references in `USER_GUIDE.md` / `BUILD_INSTRUCTIONS.md` updated to `FinTrace NCRP Setup 0.3.0.exe`. |
| 2 | Visual PDF dossier (charts + annexure split) | **[x] PASS** | Dossier upgraded from tabular-only to visual: money-flow network, layer-breakdown, and daily-volume charts built as SVG and rasterised to PNG (`@resvg/resvg-js`, `utils/charts.js`) for reliable printing; deterministic (same case → same charts). Bulky tables moved to a labelled Annexure (A–H); all Section-102 lien letters and financial figures preserved. |
| 3 | Evidentiary provenance (source-file SHA-256) | **[x] PASS** | SHA-256 of the raw uploaded NCRP file is computed before parsing (`lib/provenance.js`), stored on the case record + audit log (filename, hash, upload timestamp, app version), stamped into the PDF (cover "Source & Provenance" block, every page footer, and PDF metadata), and a changed-source warning fires when the same case is re-ingested from a file with a different hash. |
| 4 | Self-healing fuzzy parser | **[x] PASS** | `parsers/parseFuzzy.js` adds a 3rd sheet/column resolution tier behind exact + loose matching (max Dice/Levenshtein ≥ 85%). The happy path is byte-identical; any fuzzy resolution is surfaced as a `parse_warning` and recorded in the PDF Data-Quality annexure + the Excel **Parse Audit** sheet. |
| 5 | Suspected-duplicate flagging | **[x] PASS** | Exact and probable duplicate rows are flagged **non-destructively**, with raw vs. deduped counts and trail metrics surfaced in the analyzer `reconciliation` block, the PDF, and the Excel **Parse Audit** sheet. Old Transactions (>6 months) are excluded from all figures and preserved separately. |
| 6 | Native Save As for all exports (PDF + Excel) | **[x] PASS** | Packaged builds prompt a native OS "Save As" dialog **before** generating; cancel writes nothing. Backed by whitelisted IPC channels `dialog:show-save-dialog` + `file:save-as` (preload `showSaveDialog`/`saveExportAs`); `file:save-as` validates the source is inside `EXPORTS_DIR` and the destination is absolute. Browser fallback still streams a blob download. |
| 7 | Backend test suite green | **[x] PASS** | `cd backend && npm test` → **366/366 tests across 20 suites**. Validation harnesses: `accuracy_test.js` **30/30** vs CypherSOL gold; `security_audit.js` **10/10**; cross-artifact validator (`scripts/validate_v020.js`) **121 checks + 4/4 suites**; `consistency_test.js` / `e2e_validate.js` / `benchmark.js` pass. |
| 8 | Code signing | **[~] IN PROGRESS** | Authenticode signing of the installer and binaries is still being set up (carried over from 0.2.0). |
| 9 | Offline auto-updater | **[~] IN PROGRESS** | `electron-updater` is wired as a guarded stub (`checkForUpdatesAndNotify()` in `electron/main.js`); no `publish` feed is configured yet, so it is inert. Offline / air-gapped update flow remains under development. |
