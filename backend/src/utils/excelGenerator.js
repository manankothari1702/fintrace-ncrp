...
const ENTITY_SHEET_COLUMNS = {
  ...
  layer: [
    ['Account', (r) => r.account_no || ''],
    ['Bank', (r) => r.bank_name || ''],
    ['Received (gross) [Rs.]', (r) => num(r.total_received)],
    ['Traced fraud in [Rs.]', (r) => num(r.disputed_received)],
    ['Cashed out [Rs.]', (r) => num(r.total_cashout)],
    ['Mule score', (r) => num(r.mule_score)],
    ['Risk', (r) => r.risk_label || ''],
    ['Txns', (r) => num(r.txn_count)],
  ],
  edge: [
    ['Date', (r) => fmtDate(r.date)],
    ['Amount [Rs.]', (r) => num(r.amount)],
...
const ENTITY_SUMMARY_LABELS = {
  ...
  bank_count: 'Banks',
  cashout_count: 'Cash-outs',
  layer_count: 'Layers touched',
