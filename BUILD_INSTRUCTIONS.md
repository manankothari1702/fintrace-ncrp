...
(`FinTrace NCRP Setup 0.4.0.exe`) from a fresh checkout on a clean Windows
machine.
...
```
dist/
├── FinTrace NCRP Setup 0.4.0.exe   ← the installer you ship
...
```powershell
.\dist\"FinTrace NCRP Setup 0.4.0.exe"
```
...
3. Copy `FinTrace NCRP Setup 0.4.0.exe` to the VM and run it.
